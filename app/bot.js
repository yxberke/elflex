...
client.on('message', message => {
    const prefix = client.config.prefix; // prefix
...
client.config = {
    ...
    banMembers: ['864836821279965203'], // Ban yetkilileri
    banLog: '864836824124096521', // Ban Log kanal id
    jailMembers: ['864836821279965202'], // Jail yetkilileri
    jailRoles: ['864836821652733953'], // Jail rolleri
    jailLog: '864836824124096520', // Jail Log kanal id
    muteMembers: ['864836821279965199'], // Mute yetkilileri
    muteRoles: ['864836821279965197'], // Chat mute rolleri
    muteLog: '864836824124096519', // Mute log kanal id
    voicemuteRoles: ['864847874532835358'], // Ses mute rolleri
    üstYönetim: ['864836821681700874'], // Üst Yönetim rolleri
    mesajLog: '864848411308195870', // Mesaj log
    voiceChannel: '864836821681700879', // Botun gireceği ses kanal id
    boosterRoles: '', // Booster rolü, yoksa boş bırakın
    mods: ['864836821665972224'], // Moderatör rolleri
    chat: '864836822433923079', // Chat idsi
    guildID: '864836821279965196', // Sunucu id

}

client.on('ready', () => {
    client.user.setPresence({ activity: { name: 'The Lord¿ Sunar' }, status: 'idle' })
    client.channels.cache.get(client.config.voiceChannel).join() 
    console.log(`Bot ${client.user.tag} Adı İle Giriş Yaptı!`);
})

client.on('message', message => {
   if (!message.guild || message.author.bot || message.content.startsWith('.')) return;
    let embed = new Discord.MessageEmbed().setColor('#5865F2')
    if (message.mentions.users.size >= 1) {
       ...
       if (db.get(`${member.id}_sebeb`)) {
            ...
            message.channel.send(embed.setDescription(`${member} adlı üye **${db.get(`${member.id}_sebeb`)}** sebebi ile **${time}dir** afk!`)).then(x => x.delete({ timeout: 5000 }))
        }
    } else {
     if(db.get(`${message.author.id}_sebeb`)){
       db.delete(`${message.author.id}_sebeb`)
       message.channel.send(`<@${message.author.id}> Hoşgeldin artık AFK değilsin.`).then(x => x.delete({ timeout: 5000 }))
    }         
  }
})
...
client.on('message', (message) => {
  if(message.attachments.size >= 1){
   if(message.channel.id == "864836822433923079"){
...
client.on('message', async(message) => {
 if(white[message.author.id] || message.author.id == client.user.id) return;
  ...
  if (link.test(message.content)){
       db.add(`sallycode.${message.author.id}`, 1);
	   message.delete()
	 if(db.get(`sallycode.${message.author.id}`) >= 5){
...
client.on('messageDelete', (message) => {
  let channel = client.guilds.cache.get(client.config.guildID).channels.cache.get(client.config.mesajLog) //eğer kanal silinir ve koruma varsa bu kanala atar id koyarsanız hata alırsınız.
...
client.on('messageUpdate', (oldMessage, newMessage) => {
  let channel = client.guilds.cache.get(client.config.guildID).channels.cache.get(client.config.mesajLog) //log kanalının id
